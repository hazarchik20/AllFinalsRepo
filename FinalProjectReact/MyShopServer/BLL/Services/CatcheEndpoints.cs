...
namespace BLL.Services
{
    public class CatcheEndpoints
    {
        ...
        private ShopReposytory<DLL.Models.Address> _addresRepository;
        ...
        public CatcheEndpoints(DataContext context) 
        {
            ...
            _addresRepository = new ShopReposytory<DLL.Models.Address>(_context);
        ...
        public async Task<(string, int)> CatchePOST(HttpListenerRequest request)
        {
            ...
            {
                var bllOrder = JsonSerializer.Deserialize<BLL.Models.Order>(body);

                
                var address = new DLL.Models.Address
                {
                    City = bllOrder.Address.City,
                    Street = bllOrder.Address.Street,
                    HouseNumber = bllOrder.Address.HouseNumber,
                    PostalCode = bllOrder.Address.PostalCode
                };

                var newAddress = await _addresRepository.AddAsync(address);


                var order = TranslateBLLtoDLL.TranslateOrder(bllOrder);
                order.State = "CREATED";

                order.AddressId = newAddress.Id;
                await _orderRepository.AddAsync(order);

                response = JsonSerializer.Serialize(order, _jsonOptions);
                statusCode = 201;
            }
            else
            {
        ...
        public async Task<(string, int)> CatchePUT(HttpListenerRequest request)
        {
            ...
            {
                var product = TranslateBLLtoDLL.TranslateProduct(JsonSerializer.Deserialize<Product>(body));
                product.Id = int.Parse(productIdMatch.Groups[1].Value);
            ...
            else if (orderIdMatch.Success)
            {
                ...
                var order = TranslateBLLtoDLL.TranslateOrder(JsonSerializer.Deserialize<Order>(body));
        ...
        public async Task<(string, int)> CatcheDELETE(HttpListenerRequest request)
        {
            ...
            {
                if(ex.Message.Contains("An error occurred while saving the entity changes. See the inner exception for details."))
                {
                    response = "Cannot delete entity because it is referenced by other entities.";
                    statusCode = 409; 
                    return (response, statusCode);
                }
                response = ex.Message;
